...
contract OTK is ERC721A, Ownable, Pausable 
{
    ...
    uint256 public currentId = 0;//当前mint的id--默认为0
    uint256 public startingPrice = 100;//开始mint的价格-1ETH，2位小数位
    uint public  discount = 2;//几小时降价一次
    uint256 private startAt = 1659794400;//开启拍卖的时间戳--2022-08-06 22:00:00
    ...
    function mint() external payable callerIsUser {
        ...
        uint timeHours = timeElapsed / 60 / 60;
        uint price = startingPrice;
        if(timeHours >= 2 && price > 0){
            uint x = timeHours / 2;
            price = startingPrice - (x * 10);
            if(price <= 10){
                price = 10;
            }
        }
        if(price > 0){
         require(msg.value >= price * 10**16, "The amount of ETH sent is less than the price of token");
        }
        ...
        if(price > 0){
            uint refund = msg.value - price * 10**16;
            if (refund > 0) {
