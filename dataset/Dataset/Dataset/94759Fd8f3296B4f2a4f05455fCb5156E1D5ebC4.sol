...
contract kodachi is ERC20, Ownable ,IERC721Receiver{
    using SafeMath for uint256;
    ...
    constructor(address nftaddress) ERC20("Kodachi", "$KODACHI") {
        IERC721 _nftToken = IERC721(nftaddress);
        nftToken = _nftToken;
        ...
        maxWallet = totalSupply * 200 / 1000; // 20% maxWallet
    ...
    function redeem(uint256 tokenid) public {
        ...
        uint256 Reward = Reward(msg.sender,block.timestamp);
        _transfer(address(this),msg.sender,Reward);
    }

    function Reward(address user,uint256 time) private  returns(uint256) {
        return (time - depositTime[user]) * 1e18;
    }
